...
public:
  ...
  virtual double calc_PCA(const std::vector<Vector>& pos, const bool& squared , Tensor & Rotation, std::vector<Vector>   & ddistdpos ,Matrix<std::vector<Vector> > & DRotDPos, std::vector<Vector>   & alignedpositions, std::vector<Vector>  & centeredpositions ,std::vector<Vector>  & centeredreference  ) ;
